...
public final class ExpressionParser
{
    ...
    public static String processExpression(List<ExpressionNode> result, String expression)
    {
        ...
        while(changed) {
            ...
            for(int x=0;x<result.size();++x) {
                ...
                if(n.expression.startsWith("\0")) {
                    return n.expression.substring(1);
                }
                if(b) {
                    changed=true;
                    break;
     ...
     * @param node the node to expand (returns node.expression starting with "\0" 
     *   to explain any errors).
     * @return true if changes were made to the node
     */
    private static boolean processExpressionNode(List<ExpressionNode> master, ExpressionNode node) {
        ...
        // We'll apply some auto-parenthesis here for a series of 3 or
        // more connectors. We will NOT apply order of operation.
        ExpressionLeftRightOpInfo pi = null;
        while(true) {
            pi = parseLogicConnector(e);
            if(!pi.left.startsWith("\0")) {
                break;
            }
            if(!pi.left.startsWith("\0Extra information: ")) {
                node.expression = pi.left;
                return true;
            }
            int ii = e.indexOf(pi.left.substring(19));
            // We could make sure the extra starts with || or &&
            node.expression = "("+e.substring(0,ii)+")"+e.substring(ii);
            e = node.expression;            
     ...
     * @return the parsed information. The "left" field will start with "\0"
     *   and contain any error
     */
     ...
     * @return the first term in the string or error message starting with '\0'.
     */
    private static String parseTerm(String expression) 
    {
        ...
        return("\0Missing or extra parenthesis: "+expression);
    }        
