...
public class FlowPreprocessor
{
    ...
    {      
        ...
        for(int x=0;x<c.lines.size();++x) {
            ...
            if(!ccom.startsWith("IF")) {
                return co.file+":"+co.lineNumber+" Expected 'IF(exp) {'";
            }
            if(!ccom.endsWith("{")) {
                return co.file+":"+co.lineNumber+" Expected 'IF(exp) {'";
            }
            ...
            if(j==c.lines.size()) {
                return co.file+":"+co.lineNumber+" IF must be closed with a lone '}' line or '} else {'";
            }
            ...
            if(cj.text.toUpperCase().indexOf("ELSE")>=0) {
                ...
                if(jj==c.lines.size()) {
                    return co.file+":"+cj.lineNumber+" '} ELSE {' must be closed with a lone '}' line";
                }                
            }
        ...
        for(int x=0;x<c.lines.size();++x) {
            ...
            if(co.bracketType==2) {
                if(ccom.startsWith("WHILE")) {
                    ...
                    if(!co.text.endsWith("{")) {
                        return co.file+":"+co.lineNumber+" Must in with '{' like 'WHILE(exp) {'";
                    }
                    String exp = co.text.substring(5,co.text.length()-1).trim();  
                    if(!exp.startsWith("(")) {
                        return co.file+":"+co.lineNumber+" WHILE must be followed by '(' like 'WHILE(exp) {'";
                    }
                    if(!exp.endsWith(")")) {
                        return co.file+":"+co.lineNumber+" WHILE expression must be followed by ')' like 'WHILE(exp) {'";
                    }
                    ...
                    if(j==c.lines.size() || !c.lines.get(j).text.equals("}")) {
                        return co.file+":"+co.lineNumber+" WHILE must be closed with a lone '}' line";
                    }         
                    ...
                    if(!t.equals("DO")) {
                        return co.file+":"+co.lineNumber+" Expected 'DO {'";
                    }
                    // Find the ending bracket for the WHILE
                    int j = x;
                    while(j<c.lines.size()) {
                    ...
                    if(!ct.startsWith("WHILE")) {
                        return cj.file+":"+cj.lineNumber+" Expected '} WHILE(exp)'";
                    }
                    ct = t.substring(5).trim();
                    if(!ct.startsWith("(")) {
                        return cj.file+":"+cj.lineNumber+" Expected '(' in '} WHILE(exp)'";
                    }
                    if(!ct.endsWith(")")) {
                        return cj.file+":"+cj.lineNumber+" Expected ')' in '} WHILE(exp)'";
                    }
                    ...
                    if(!t.startsWith("(") || !t.endsWith(")")) {
                        return co.file+":"+co.lineNumber+" Expected 'FOR(init;exp;inc) {'";
                    }
                    ...
                    if(j==c.lines.size() || !c.lines.get(j).text.equals("}")) {
                        return co.file+":"+co.lineNumber+" FOR must be closed with a lone '}' line";
                    }    
                    // This is the ending bracket line
                    CodeLine cj = c.lines.get(j);
                    c.lines.add(j+1,new CodeLine(cj.lineNumber,cj.file,"_loop_"+loopNumber+"_end:"));
    ...
    public String markLevels(Cluster c)
    {
        ...
        for(int x=0;x<c.lines.size();++x) {
            CodeLine co = c.lines.get(x);
            co.bracketLevel = lev;
            if(co.text.toUpperCase().equals("--DATA--")) {
                break; // No code in the data section
            }
            ...
            if(lev<0) {
                return co.file+":"+co.lineNumber+" Too many close brackets";
            }            
