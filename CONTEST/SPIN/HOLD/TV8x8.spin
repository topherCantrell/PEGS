...
' 2 row overlay mask and image at 0080 (mask) and 0100 (image)
...
' Field
'
field                   rdlong  x,C_RETCNT              ' Bump the ...
                        add     x,#1                    ' ... vertical retrace ...
                        wrlong  x,C_RETCNT              ' ... counter
...
:black                  call    #hsync                  'do hsync
                        ...
                        mov     onum,#0
                        mov     overlayMask,#SorEvenMask
                        mov     ovImage,#SorEvenImage     
...
:tile                   rdword  tile,screen             'read tile                           
                        ...
                        rol     tile,#4                 'read tile pixels
                        mov     colorHold,tile
                        and     tile,C_FFF   
                        add     tile,_tileMemory                     
                        rdword  pixels,tile             '(2 instructions between reads)
                        
                        add     screen,#2         ' Repeat the lookup for 2nd tile
                        rdword  tile2,screen
                        and     tile2,C_FFF                        
                        or      tile2,line
                        rol     tile2,#4
                        ...
                        rdlong  omask,overlayMask                          
                        add     overlayMask,#4
                        cmp     overlayMask,#SorEvenImage wz                                             

                        rdlong  oimage,ovImage                          
                        add     ovImage,#4                         
                        
                    if_z mov     overlayMask,#SorEvenMask
                    if_z mov     ovImage,#SorEvenImage wz ' Note we reset the Z flag here like it was coming in
                        ...
                        ' Both tiles share the color scheme of the first tile
                        mov     tile,colorHold
                        shr     tile,#12+4              'set tile colors                        
...
overlayMask      long  SorEvenMask       
ovImage          long  SorEvenImage
omask    long 0
oimage   long 0
onum     long 0
overlayRowNum    long  DisplayBeamRowNumber

C_CLKFREQ long $04_C4_B4_00 ' Hardcoded clock frequency for the PEGS project
C_FFF     long $0FFF
C_RETCNT  long RetraceCounter
...
_pins                   long 0       '%pppmmmm       read-only
...
_hx                     long 0       '4+             read-only
_vx                     long 0       '1+             read-only
_ho                     long 0       '0+-            read-only
_vo                     long 0       '0+-            read-only
_broadcast              long 0       '0+             read-only
_tileMemory             long TileMemory       
...
''
''___
''VAR                   'TV parameters - 15 contiguous longs
''
...
''      each word has two bitfields: a 4-bit colorset ptr and a 12-bit pixelgroup ptr
''      %ccccpppppppppppp
''        c (0-15) is an index into table on the COG. Each word defines 4 colors.
''        p is a pointer that gets shifted 4 places left (thus the 16-byte boundary)
''
''    pointer to words which define screen contents (left-to-right, top-to-bottom)
''      number of words must be tv_ht * tv_vt
...
''    in PAL, the offset frequency and max bandwidth vary by PAL