...
CON
  ...
  CACHE_SIZE = 10 ' As big as possible
...
VAR
  ...
  byte   cluster[CACHE_SIZE*2048]     
  long   clusterContent[CACHE_SIZE]
  byte   sticky[CACHE_SIZE]
  byte   lru[CACHE_SIZE]
...
PUB cacheCluster(clusterNum) | i,j,k, t

  ' See if the request is already cached
  repeat i from 0 to (CACHE_SIZE-1)
    if (clusterContent[i] == clusterNum)      
      'Don't do anything if this is already on top
      if(lru[0]==i)
        return @cluster[2048*i]       
      'We must be in the LRU list ... find us
      repeat t from 0 to (CACHE_SIZE-1)
        if(lru[t]==i)
          'Move everybody above us down
           repeat j from t to 1 step -1
             lru[j] := lru[j-1]
          'Put us on top
          lru[0] := i         
      return @cluster[2048*i]      
  ...
  repeat i from 0 to (CACHE_SIZE-1)
    if (clusterContent[i] == $FFFF)
      ...
      repeat j from (CACHE_SIZE-1) to 1 step -1
        lru[j] := lru[j-1]
      lru[0] := i     
      return @cluster[2048*i]

  ' Pull the index from the end of the LRU list and eject its contents
  
  ' Look for the LRU that is non-sticky
  repeat j from (CACHE_SIZE-1) to 1 step -1
    if(sticky[j]==0)
      ' Close over the slot we are ejecting
      repeat k from j to 1 step -1
        lru[k] := lru[k-1]
      ' Read the data from disk
      repeat i from 0 to 3
        readSector(firstDataSector+clusterNum*4+i, @cluster+2048*j+512*i)  
      clusterContent[j] := clusterNum
      lru[0] := clusterNum
      return @cluster[2048*j]
