PUB start 
  ...
  coginit(2,@VideoCOG,0)

CON

MailboxNumber = 2            ' Our mailbox number

ScreenMemory = $0200         ' Where tile-map begins in memory
TileMemory   = $0880         ' Where tile-images begin in memory (16 bytes each tile)

DisplayBeamRowNumber = $7812 ' Where the TV8x8 records what row it is drawing
...
commandTable
  ...
  ' Each command is passed i=varForm, tmp=complete command long
         ...
         jmp  #doSETCURSORINFO ' 00101
         jmp  #doGETNUMBER     ' 00110
         jmp  #doGETLINE       ' 00111
...
' --------------------------------------------------------------------------------
' [00110] GETNUMBER Vn
'   1_bbb_0000____00_0_00110__00000000_vvvvvvvv
'
doGETNUMBER
         mov       t2,tmp              ' Remember ...
         and       t2,#$FF             ' ... the register         
         
         mov       p,C_SCRATCH         ' Scratch area to store number
         mov       x,#$30              ' Only ...
         mov       y,#$39              ' ... numbers
         mov       width,#10           ' Max of 10 digits
         call      #readBuf            ' Read the buffer
doGTOUT  cmp       t1,#0 wz            ' Set to 0 ...
    if_z mov       tmp,#0              ' ... if nothing ...
    if_z jmp       #doGTT2             ' ... was entered
         mov       p,C_SCRATCH         ' Scratch buffer
         mov       tmp,#0              ' Starting point
doGTT1   rdbyte    data2,p             ' Get next character
         add       p,#1                ' Bump the buffer pointer
         mov       data1,tmp           ' Hold the X
         shl       tmp,#2              ' X = X * 4
         add       tmp,data1           ' Now * 5
         shl       tmp,#1              ' Now * 10
         sub       data2,#$30          ' Convert from ascii
         add       tmp,data2           ' Add in the digit         
         djnz      t1,#doGTT1          ' Do all digits

         
                  
doGTT2   call      #storeVar           ' Store the variable
         jmp       #final              ' Done

' --------------------------------------------------------------------------------
' [00111] GETLINE ptr,size
'   1_111_0bbb____00_0_00111__pppppppp_pppppppp
'   00000000_00000000_ssssssss_ssssssss
'
doGETLINE
          rdlong   com,boxOfs          ' Cluster offset          
          mov      p,tmp               ' Get ...
          and      p,C_FFFF            ' ... buffer pointer          
          add      p,com               ' Absolute address
          rdlong   width,boxDat1Ret    ' Get ...
          and      width,C_FFFF        ' ... width
          mov      x,#32               ' Range is ...
          mov      y,#127              ' ... all characters
          call     #readBuf            ' Fill the buffer
          jmp      #final              ' Done
...
' --------------------------------------------------------------------------------
' [00101] SETCURSORNFO rate,tileA,tileB
'   1_111_0bbb____00_0_00101__rrrrrrrr_rrrrrrrr
'   AAAAAAAA_AAAAAAAA_BBBBBBBB_BBBBBBBB
'
doSETCURSORINFO
         mov       curRate,tmp         ' Get the ...
         and       curRate,C_FFFF      ' ... cursor ...
         'shl       curRate,#16         ' ... blink rate
         mov       curCnt,#1           ' Reload next blink
         rdlong    tileB,boxDat1Ret    ' Get ...
         mov       tileA,tileB         ' ... tileB ...
         and       tileB,C_FFFF        ' ... and ...
         shr       tileA,#16           ' ... tileA
         jmp       #final              ' Done
...
doGETCUE shl       y,#6                ' *64 bytes per row
         ...
         jmp       #doGTT2             ' Store value (tmp) to variable (t2)
...
' --------------------------------------------------------------------------------
' Reads keys into a buffer
' p = buffer pointer
' x = valid lower character
' y = valid upper character
' width = size of buffer
'
readBuf  mov       t1,#0               ' Clear running count
readBuf1 call      #cursorBlink        ' Blink cursor
         mov       cStat,C_KEYLKA      ' Get ...
         mov       data1,C_KEYLKB      ' ... next ...         
         mov       box,#1
         call      #cogTalk            ' ... key           
         cmp       data1,#0 wz         ' Loop back ...          
    if_z jmp       #readBuf1           ' ... if none pressed
         wrbyte    tileA,textCursor    ' Cursor off
         
         cmp       data1,#13 wz        ' If key is ENTER ...
    if_z mov       data1,#0            ' Null terminate ...
    if_z wrbyte    data1,p             ' ... the buffer
readBuf_ret
    if_z ret                           ' Done

         cmp       data1,#200 wz       ' Ignore if not ...
   if_nz jmp       #rb1                ' ... BACKSPACE
         cmp       t1,#0 wz            ' Ignore BS if ...
    if_z jmp       #readBuf1           ' ... buffer is empty
         sub       t1,#1               ' Remove last key ...
         sub       p,#1                ' ... from scratch
         mov       tmp,#8              ' Remove last key ...
         call      #prChar             ' ... from screen
         jmp       #readBuf1           ' Loop back 
                
rb1      cmp       data1,x wz,wc       ' Ignore ...
    if_b jmp       #readBuf1           ' ... key ...
         cmp       data1,y wz,wc       ' ... if it ...
    if_a jmp       #readBuf1           ' ... is not a valid
         cmp       t1,width wz         ' Ignore if ...
    if_z jmp       #readBuf1           ' ... buffer is full
         wrbyte    data1,p             ' Put the key ...
         add       t1,#1               ' ... in the ...
         add       p,#1                ' ... buffer
         mov       tmp,data1           ' Print ...
         call      #prChar             ' ... the key                    
...
baseBox    long    MailboxBase  ' The base of all boxes   
...
textCursor long    ScreenMemory        ' Current text cursor (pointer into tile-map memory)
...
C_SCRNOV   long    32*26*2+ScreenMemory    ' Screen map + 1 entry (overflow check)
...
C_SCRMEM   long    ScreenMemory          ' Where the tile map is stored
C_TILEMEM  long    TileMemory            ' Where tile images are stored
...
' Our mailbox
boxComStat long    MailboxBase + MailboxNumber*32
boxDat1Ret long    MailboxBase + MailboxNumber*32 +4
boxDat2    long    MailboxBase + MailboxNumber*32 +8
boxDat3    long    MailboxBase + MailboxNumber*32 +12
boxDat4    long    MailboxBase + MailboxNumber*32 +16
'boxDat5   long     MailboxBase + MailboxNumber*32 +20
'boxDat6   long     MailboxBase + MailboxNumber*32 +24
boxOfs     long    MailboxBase + MailboxNumber*32 +28
